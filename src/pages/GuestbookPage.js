...
// ────────────────────────────────────────────────────────
// 공개 페이지용 GuestbookView (ownerId prop 있을 때)
// ────────────────────────────────────────────────────────
export function GuestbookPublicView({ ownerId }) {
  ...
  const submitPage = async () => {
    ...
    const usernameMatch = pageForm.url.match(/\/u\/([^/?#\s]+)/)
    setPageSubmitting(true)
    await supabase.from('guestbook').insert({
      owner_id: ownerId,
      author_id: user?.id || null,
      author_name: pageForm.nickname.trim(),
      author_username: usernameMatch ? usernameMatch[1] : null,
  ...
  const addEmoji = async (id, emoji) => {
    ...
    const cleanName = item.author_name.replace(/^[\p{Emoji}\s]+/u, '').trim()
    await supabase.from('guestbook').update({ author_name: `${emoji} ${cleanName}` }).eq('id', id)
...
export function GuestbookPage({ ownerId }) {
  ...
  // ownerId가 넘어오면 공개 페이지용으로 사용
  const targetId = ownerId || user?.id
  const isOwner = !!(user && targetId && user.id === targetId)

  const load = async () => {
    if (!targetId) return
    setLoading(true)
    const { data: all } = await supabase
      .from('guestbook').select('*').eq('owner_id', targetId)
      .order('created_at', { ascending:false })
    setMessages((all||[]).filter(g => g.type==='message'||!g.type))
    setMypages((all||[]).filter(g => g.type==='mypage'))
    setLoading(false)
        ...
        loading
          ...
          : messages.length === 0
            ? <div className="card" style={{textAlign:'center',padding:40,color:'var(--color-text-light)',fontSize:'0.85rem'}}>
                아직 방명록이 없어요.<br/>
                <span style={{fontSize:'0.8rem'}}>내 공개 페이지를 공유하면 방문자들이 방명록을 남길 수 있어요!</span>
              </div>
            : <div style={{ display:'flex', flexDirection:'column', gap:10 }}>
                {messages.map(g => (
                  <div key={g.id} className="card" style={{ padding:'14px 20px' }}>
        ...
        <div>
          ...
          {loading
            ...
            : mypages.length === 0
              ? <div className="card" style={{textAlign:'center',padding:40,color:'var(--color-text-light)',fontSize:'0.85rem'}}>
                  아직 남긴 페이지가 없어요.<br/>
                  <span style={{fontSize:'0.8rem'}}>방문자들이 내 공개 페이지에서 자신의 링크를 남길 수 있어요!</span>
                </div>
              : <div style={{ display:'flex', flexWrap:'wrap', gap:10 }}>
                  {mypages.map(g => {
                    const href = g.content?.startsWith('http') ? g.content
