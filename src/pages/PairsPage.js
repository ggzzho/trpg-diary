...
export function PairsPage() {
  ...
  const save = async () => {
    if (!form.name) return
    if (!editing && items.length >= 3000) { alert('게시판의 최대 등록 갯수를 초과하여 저장할 수 없습니다. 페어 목록을 정리해주세요.'); return }
    ...
    <div className="fade-in">
      ...
      >
        ...
        <div className="form-group">
          <label className="form-label">페어 이미지 <span style={{fontWeight:400,color:'var(--color-text-light)',fontSize:'0.78rem'}}>(권장: 정사각형, 200×200px 이상)</span></label>
