...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      setUpcoming((s.data||[]).filter(x=>x.scheduled_date>=todayStr&&x.status!=='cancelled').sort((a,b)=>a.scheduled_date.localeCompare(b.scheduled_date)).slice(0,5))
  ...
  const STAT_CARDS = [
    ...
    {label:'시나리오 목록', value:stats.scenarios, icon:'🗺️', to:'/scenarios', unit:'개'},
          ...
          {upcoming.length===0
            ...
            : upcoming.map(s=>(
              <div key={s.id} style={{padding:'10px 0',borderBottom:'1px solid var(--color-border)',display:'flex',gap:12,alignItems:'flex-start'}}>
                ...
                <div>
                  <div style={{fontWeight:500,fontSize:'0.88rem'}}>{s.title}</div>
          ...
          <div style={{display:'flex',gap:10,flexWrap:'wrap'}}>
            {favorites.map(f=>(
              <a key={f.id} href={`/u/${f.target_username}`} target="_blank" rel="noreferrer"
                ...
                <div style={{width:24,height:24,borderRadius:'50%',background:'var(--color-primary)',display:'flex',alignItems:'center',justifyContent:'center',fontSize:'0.72rem',color:'white',fontWeight:700,flexShrink:0}}>
                  {f.target_avatar_url ? <img src={f.target_avatar_url} alt="" style={{width:'100%',height:'100%',borderRadius:'50%',objectFit:'cover'}} /> : (f.target_display_name||'?')[0]}
                </div>
