...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      setStats({ schedules:s.data?.length||0, logs:l.data?.length||0, rulebooks:r.data?.length||0, scenarios:sc.data?.length||0, pairs:p.data?.length||0 })
      const today = new Date().toISOString().split('T')[0]
      setUpcoming((s.data||[]).filter(x=>x.scheduled_date>=today&&x.status!=='cancelled').sort((a,b)=>a.scheduled_date.localeCompare(b.scheduled_date)).slice(0,3))
    ...
    <div className="fade-in">
      ...
      <div className="grid-2">
        ...
        <div className="card">
          <div className="flex justify-between items-center" style={{marginBottom:20}}>
            <h2 className="text-serif" style={{color:'var(--color-accent)',fontSize:'1.1rem'}}>📅 다가오는 일정</h2>
            <Link to="/schedule" className="btn btn-ghost btn-sm">전체 보기</Link>
          </div>
          {upcoming.length===0
            ? <p className="text-light text-sm">예정된 일정이 없어요</p>
            : upcoming.map(s=>(
              <div key={s.id} style={{padding:'12px 0',borderBottom:'1px solid var(--color-border)',display:'flex',gap:12,alignItems:'flex-start'}}>
                {/* 날짜 박스 - 테마 컬러 적용 */}
                <div style={{
                  ...
                  <div className="text-serif" style={{fontSize:'1.2rem',color:'white',fontWeight:700}}>
        ...
        <div className="card">
          <div className="flex justify-between items-center" style={{marginBottom:20}}>
            <h2 className="text-serif" style={{color:'var(--color-accent)',fontSize:'1.1rem'}}>📖 최근 기록</h2>
            <Link to="/logs" className="btn btn-ghost btn-sm">전체 보기</Link>
          </div>
          {recentLogs.length===0
            ? <p className="text-light text-sm">아직 기록이 없어요</p>
            : recentLogs.map(l=>(
              <div key={l.id} style={{padding:'10px 0',borderBottom:'1px solid var(--color-border)',display:'flex',justifyContent:'space-between',alignItems:'center'}}>
                <div>
                  <div style={{fontWeight:500,fontSize:'0.88rem'}}>{l.title}</div>
                  <div className="text-xs text-light">
                    {l.played_date&&format(new Date(l.played_date),'yyyy.MM.dd')} · {l.role}
                  </div>
                </div>
                {l.rating>0&&(
      ...
      {profile&&(
        <div className="card" style={{marginTop:24,display:'flex',justifyContent:'space-between',alignItems:'center'}}>
          <div>
            <div className="text-serif" style={{color:'var(--color-accent)',marginBottom:4}}>🔗 내 공개 페이지</div>
            <div className="text-sm text-light">{window.location.origin}/u/{profile.username}</div>
          </div>
          <button className="btn btn-outline btn-sm" onClick={()=>{navigator.clipboard.writeText(`${window.location.origin}/u/${profile.username}`);alert('링크가 복사되었어요!')}}>
            링크 복사
          </button>
        </div>
      )}
