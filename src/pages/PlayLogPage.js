    ...
    <div className="fade-in">
      ...
      {loading ? <LoadingSpinner /> : items.length === 0
        ...
        : <div style={{display:'flex',flexDirection:'column',gap:10}}>
            {items.map(item => (
              <div key={item.id} className="card card-sm" style={{display:'flex',gap:14,alignItems:'flex-start',cursor:'pointer'}} onClick={()=>setDetail(item)}>
                <div style={{background:'rgba(200,169,110,0.1)',borderRadius:8,padding:'7px 11px',textAlign:'center',minWidth:48,flexShrink:0}}>
                  <div style={{fontSize:'0.62rem',color:'var(--color-text-light)'}}>{format(new Date(item.played_date),'yyyy')}</div>
                  <div style={{fontSize:'0.95rem',color:'var(--color-accent)',fontWeight:700}}>{format(new Date(item.played_date),'M/d')}</div>
                </div>
                <div style={{flex:1}}>
                  ...
                  {item.rating>0 && <div className="stars" style={{fontSize:'0.8rem',marginTop:3}}>{'★'.repeat(item.rating)}{'☆'.repeat(5-item.rating)}</div>}
                </div>
        ...
        {detail && (
          <div>
            <div className="grid-2" style={{marginBottom:14}}>
              <div><div className="form-label">날짜</div><div className="text-sm">{format(new Date(detail.played_date),'yyyy년 M월 d일')}</div></div>
              ...
              {detail.scenario_name && <div><div className="form-label">시나리오</div><div className="text-sm">{detail.scenario_name}</div></div>}
              {detail.character_name && <div><div className="form-label">캐릭터명</div><div className="text-sm">{detail.character_name}</div></div>}
              {detail.gm_name && <div><div className="form-label">GM</div><div className="text-sm">{detail.gm_name}</div></div>}
              {detail.venue && <div><div className="form-label">툴</div><div className="text-sm">{detail.venue}</div></div>}
            </div>
      ...
      >
        ...
        <div className="form-group">
          ...
          <input className="form-input" placeholder="roll20, 코코포리아..." value={form.venue} onChange={set('venue')} />
        </div>
        <div className="form-group">
