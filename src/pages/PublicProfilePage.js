  ...
  return (
    <div style={{maxWidth:820,margin:'0 auto',padding:'36px 20px'}}>
      ...
      <div className="card card-lg" style={{marginBottom:28,textAlign:'center'}}>
        ...
        {/* 플레이 스타일 / 주의사항 / 기타 */}
        {(profile.play_style||profile.caution||profile.extra_info) && (
          <div style={{marginTop:16,textAlign:'left',display:'flex',flexDirection:'column',gap:12}}>
            {profile.play_style && (
              <div>
                <div style={{fontSize:'0.72rem',fontWeight:700,color:'var(--color-accent)',marginBottom:4}}>플레이 스타일</div>
                <p style={{fontSize:'0.84rem',color:'var(--color-text-light)',lineHeight:1.7,whiteSpace:'pre-wrap'}}>{profile.play_style}</p>
              </div>
            )}
            {profile.caution && (
              <div>
                <div style={{fontSize:'0.72rem',fontWeight:700,color:'var(--color-accent)',marginBottom:4}}>주의 사항</div>
                <p style={{fontSize:'0.84rem',color:'var(--color-text-light)',lineHeight:1.7,whiteSpace:'pre-wrap'}}>{profile.caution}</p>
              </div>
            )}
            {profile.extra_info && (
              <div>
                <div style={{fontSize:'0.72rem',fontWeight:700,color:'var(--color-accent)',marginBottom:4}}>기타 사항</div>
                <p style={{fontSize:'0.84rem',color:'var(--color-text-light)',lineHeight:1.7,whiteSpace:'pre-wrap'}}>{profile.extra_info}</p>
              </div>
            )}
          </div>
        )}
              ...
              <div key={s.id} className="card card-sm" style={{display:'flex',alignItems:'center',gap:14}}>
                ...
                <div style={{flex:1}}>
                  <div className="flex items-center gap-8" style={{marginBottom:3}}>
                    ...
                    <span className={`badge ${SCENARIO_STATUS[s.status]==='미플'?'badge-gray':s.status==='want'?'badge-blue':'badge-green'}`}>{SCENARIO_STATUS[s.status]}</span>
                  </div>
                  <div className="text-xs text-light flex gap-10">
                    {s.system_name&&<span>🎲 {s.system_name}</span>}
