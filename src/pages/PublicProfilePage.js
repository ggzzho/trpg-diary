...
export default function PublicProfilePage() {
  ...
  return (
    <div style={{maxWidth:820,margin:'0 auto',padding:'0 0 40px'}}>
      ...
      {/* ── 헤더 이미지 ── */}
      <div style={{
        height: profile.header_image_url ? 200 : 80,
        background: profile.header_image_url ? 'transparent' : 'var(--color-nav-active-bg)',
        position:'relative', overflow:'hidden', marginBottom: 0
      }}>
        {profile.header_image_url && (
          <img src={profile.header_image_url} alt="header" style={{width:'100%',height:'100%',objectFit:'cover'}} />
        )}
        {/* 헤더 그라데이션 오버레이 */}
        {profile.header_image_url && (
      ...
      <div style={{padding:'0 20px'}}>
        <div className="card card-lg" style={{marginTop: profile.header_image_url ? -40 : 0, marginBottom:28, position:'relative', zIndex:1}}>
          <div style={{textAlign:'center'}}>
            {/* 아바타 */}
            <div style={{marginTop: profile.header_image_url ? -20 : 0, marginBottom:14, display:'flex', justifyContent:'center'}}>
              <div className="user-avatar" style={{width:72,height:72,fontSize:'1.8rem',border:'3px solid var(--color-bg)'}}>
