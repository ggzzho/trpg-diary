    ...
    <div className="fade-in">
        ...
        : <div className="grid-auto">
            {items.map(item => (
              <div key={item.id} className="card">
                ...
                {item.contact && <div className="text-sm" style={{marginTop:7}}>📬 {item.contact}</div>}
              </div>
            ))}
          </div>
      }
      ...
      >
          ...
          <div className="flex gap-8" style={{flexWrap:'wrap'}}>
            {DAYS.map(d=>(
              <button key={d} type="button"
                ...
                onClick={()=>toggleDay(d)} style={{minWidth:34,justifyContent:'center'}}
              >{d}</button>
            ))}
          </div>
        </div>
        ...
        <div className="form-group">
          ...
          <textarea className="form-textarea" placeholder="원하는 장르, 분위기, 경험 등..." value={form.description} onChange={set('description')} style={{minHeight:72}} />
        </div>
        <div className="form-group">
