...
const BLANK = { title:'', system_name:'', author:'', publisher:'', cover_image_url:'', player_count:'', estimated_time:'', format:'physical', status:'unplayed', memo:'', purchase_date:'' }
    ...
    <div className="fade-in">
            ...
            {filtered.map(item=>(
              <div key={item.id} className="card card-sm" style={{display:'flex',alignItems:'center',gap:14}}>
                ...
                <div style={{flex:1}}>
                  ...
                  <div className="text-xs text-light flex gap-10" style={{flexWrap:'wrap'}}>
                    {item.system_name&&<span>🎲 {item.system_name}</span>}
                    {item.author&&<span>✏️ {item.author}</span>}
                    {item.player_count&&<span>👥 {item.player_count}</span>}
      ...
      >
        ...
        <div className="grid-2">
          ...
          <div className="form-group"><label className="form-label">작가</label><input className="form-input" value={form.author||''} onChange={set('author')} /></div>
        </div>
        <div className="grid-2">
          ...
          <div className="form-group"><label className="form-label">예상 플레이 시간</label><input className="form-input" placeholder="4~6시간" value={form.estimated_time||''} onChange={set('estimated_time')} /></div>
        </div>
        <div className="grid-2">
          <div className="form-group"><label className="form-label">형태</label>
            <select className="form-select" value={form.format} onChange={set('format')}><option value="physical">실물</option><option value="digital">전자</option><option value="both">둘 다</option></select>
          </div>
